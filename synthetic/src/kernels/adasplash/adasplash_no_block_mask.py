...
def _get_tau(
    ...
    ## -- Load ALiBi slope for this head if enabled --
    alibi_slope = 0.0
    if USE_ALIBI:
    ...
    for c_block in range(0, valid_nblocks):
        ...
        if USE_ALIBI:
            position_diff = k_idxs[None, :] - q_idxs[:, None]
            if IS_CAUSAL:
                position_diff = tl.where(causal_mask, position_diff, 0)
            if IS_VARLEN:
                position_diff = tl.where(k_mask[None, :], position_diff, 0)
            alibi_bias = alibi_slope * position_diff
            qk = qk + alibi_bias
    ...
    for c_block in range(0, valid_nblocks):
        ...
        if USE_ALIBI:
            ...
            if IS_CAUSAL:
                position_diff = tl.where(causal_mask, position_diff, 0)
            if IS_VARLEN:
                position_diff = tl.where(kv_mask[None, :], position_diff, 0)
            alibi_bias = alibi_slope * position_diff
            qk = qk + alibi_bias
...
def _bwd_kv_kernel(
    ...
    for c_block in range(start_block, valid_mblocks):
        ...
        ## -- Apply ALiBi if enabled --
        if USE_ALIBI:
            position_diff = kv_idxs[None, :] - q_idxs[:, None]
            if IS_CAUSAL:
                position_diff = tl.where(causal_mask, position_diff, 0)
            if IS_VARLEN:
                position_diff = tl.where(q_mask[:, None], position_diff, 0)
            alibi_bias = alibi_slope * position_diff
            qk = qk + alibi_bias
...
def ASSERT_VARLEN(varlen, N_CTX):
    if varlen is None:
        assert (N_CTX & (N_CTX - 1)) == 0, "If varlen is not used, the context length must be a power of two."
    else:
...
class _sparse_attention(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, q, k, v, alpha=1.5, is_causal=False, varlen=None, niter=10, alibi_slopes=None):
        # shape constraints
        ...
        ## -- Generate ALiBi slopes if needed --
        if USE_ALIBI:
            # assert that alibi_slopes is a tensor of shape (N_H,)
            alibi_slopes = alibi_slopes.view(-1)
            assert alibi_slopes.shape == (N_H,), "ALiBi slopes should be of shape (N_H,)"
    ...
    @staticmethod
    def backward(ctx, do):
        q, k, v, o, taus, varlen, alibi_slopes = ctx.saved_tensors
...
def generate_alibi_slopes(n_heads, dtype=torch.float32, device="cuda"):
    """
    Generate ALiBi slopes (2^{-8 * i/num_heads}) for each head
    """
    slopes = torch.exp2(-8 * torch.arange(1, n_heads + 1, device=device) / n_heads).to(dtype)
    return slopes


def generate_linear_alibi_slopes(n_heads, a=2, dtype=torch.float32, device="cuda"):
    """
    Generate linear ALiBi slopes max(0, 1 - a * i/num_heads) for each head
    """
    slopes = torch.relu(1 - a * torch.arange(n_heads, device=device) / n_heads).to(dtype)
    return slopes


def sparse_attn(q, k, v, alpha=1.5, is_causal=False, varlen=None, niter=5, alibi_slopes=None):
    """
    Performs sparse attention without block mask.

    Args:
        q, k, v: Query, key, and value tensors (shape: (batch_size, num_heads, seq_len, head_dim))
        alpha: Entmax alpha parameter (default: 1.5)
        is_causal: Whether to use causal attention masking (default: False)
        varlen: Variable sequence length tensor (default: None)
        niter: Number of iterations for tau calculation (default: 10)
        alibi_slopes: ALiBi slopes for each head (default: None).
                      Should be a tensor of shape (num_heads,).
                        If None, ALiBi is not applied.
                        If 'exp', slopes are generated as 2^{-8 * i/num_heads}.
                        If 'relu', slopes are generated as max(0, 1 - 2 * i/num_heads).
    Returns:
        Output tensor after attention
    """
    if alibi_slopes == 'exp':
        alibi_slopes = generate_alibi_slopes(q.shape[1], dtype=q.dtype, device=q.device)
    if alibi_slopes == 'relu':
        alibi_slopes = generate_linear_alibi_slopes(q.shape[1], a=2, dtype=q.dtype, device=q.device)
    return _sparse_attention.apply(q, k, v, alpha, is_causal, varlen, niter, alibi_slopes)
