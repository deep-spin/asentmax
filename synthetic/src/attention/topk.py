    ...
    def forward(ctx, Q, K, V, activation, mask=None, args=None):
        ...
        if topk > 0:
            ...
            attn = dots.zero_().scatter_(
                -1,
                top_inds,
                activation(top_dots, record_rng=activation.training).to(Q.dtype)
            )
    ...
    def backward(ctx, d_out):
        ...
        if topk > 0:
            ...
            with torch.enable_grad():
                ...
                top_attn = activation(top_dots, set_rng=True).to(Q.dtype)  # [Lq, topk]
...
class AttentionNoCache(nn.Module):
    ...
    def forward(self, Q, K, V, mask=None, causal_masking=False, args=None):
        ...
        for chunk_ids in torch.arange(Lq, device=Q.device).chunk(Q_chunks):
            ...
            out[..., chunk_ids, :] = _AttentionNoCache.apply(Q[..., chunk_ids, :], K, V, self.activation, chunk_mask,
                                                             args)  # [Cq, d]
        return out  # [Lq,d]
...
class _AttentionNoCacheSparse(torch.autograd.function.Function):
    ...
    def forward(ctx, Q, K, V, activation, mask=None, args=None):
        ...
        assert args is not None and 'topk' in args and 0 < args['topk'] <= K.shape[
            -2], 'this method requires 0 < topk << K.shape[-2]'
        topk = args['topk']
    ...
    def backward(ctx, d_out):
        ...
        d_top_attn = d_out.matmul(V.transpose(-2, -1)).gather(-1,
                                                              top_inds)  # [Lq, topk] <-- [Lq, Lk] == [Lq, d] x [d, Lk]
    ...
    def forward(self, Q, K, V, mask=None, causal_masking=False, args=None):
        ...
        for chunk_ids in torch.arange(Lq, device=Q.device).chunk(Q_chunks):
            ...
            if mask is not None:
                ...
                chunk_mask = torch.triu(torch.ones(len(chunk_ids), K.shape[-2], device=Q.device, dtype=torch.bool),
                                        diagonal=1 + chunk_ids[0])  # [Cq, Lk]
            out[..., chunk_ids, :] = _AttentionNoCacheSparse.apply(Q[..., chunk_ids, :], K, V, self.activation,
                                                                   chunk_mask, args)  # [Cq, d]
        return out  # [Lq,d]
